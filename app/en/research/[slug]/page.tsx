...
export default async function ResearchDetailEn({ params }: Props) {
  ...
  return (
    <div className="grid gap-6">
      ...
      <header>
        <h1 className="text-3xl font-semibold tracking-tight">{item.title_en}</h1>
        <p className="mt-2 text-zinc-600 dark:text-zinc-400">
          {item.authors.join(", ")} · {item.year}
        </p>
      </header>

      <p className="text-zinc-800 dark:text-zinc-200">{item.abstract_en}</p>
